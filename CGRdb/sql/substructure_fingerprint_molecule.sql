...
"{schema}".cgrdb_search_substructure_fingerprint_molecules(fingerprint integer[], signature bytea, OUT id integer, OUT count integer)
AS $$
...
found = plpy.execute(f'''INSERT INTO
"{schema}"."MoleculeSearchCache"(signature, operator, date, molecules, tanimotos)
SELECT '\\x{sg}'::bytea, 'substructure', CURRENT_TIMESTAMP, array_agg(o.m), array_agg(o.t)
FROM (
    SELECT h.m, h.t
    FROM (
