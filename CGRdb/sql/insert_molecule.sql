...
elif not data['is_canonic']:
    data['is_canonic'] = True

data['fingerprint'] = mfp._transform_bitset([molecule])[0]
data['signature'] = bytes(molecule)

if current and data['is_canonic']:  # change canonic structure. only after fp generation
    plpy.execute('UPDATE "{schema}"."MoleculeStructure" SET is_canonic = false WHERE id = %d' % current[0]['id'])
